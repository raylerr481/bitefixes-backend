"""Unified inbound channel adapters for Bitey."""
...
def normalize_event(channel: str, payload: dict[str, Any]) -> dict[str, Any] | None:
    if channel == "telegram":
        msg = payload.get("message") or payload.get("edited_message") or {}; sender = msg.get("from") or {}; text = _text(msg.get("text"))
    ...
    if channel == "messenger":
        entry = (payload.get("entry") or [{}])[0]; messaging = (entry.get("messaging") or [{}])[0]; message = messaging.get("message") or {}; text = _text(message.get("text"))
        if not text: return None
    ...
    if channel == "whatsapp":
        entry = (payload.get("entry") or [{}])[0]; change = ((entry.get("changes") or [{}])[0]).get("value") or {}; messages = change.get("messages") or []
        if not messages: return None
        msg = messages[0]; text = _text((msg.get("text") or {}).get("body"))
        if not text: return None
        contact = (change.get("contacts") or [{}])[0]; profile = contact.get("profile") or {}; phone = _text(msg.get("from") or contact.get("wa_id"))
...
async def _handle(channel: str, request: Request):
    _verify_token(channel, request); payload = await request.json(); event = normalize_event(channel, payload)
    if not event: return {"status": "ignored", "channel": channel}
    ...
    return {"status": "processed", "channel": channel, "external_conversation_id": event["conversation_id"], "conversation_id": result.get("conversation_id"), "customer_id": result.get("customer_id"), "response": result.get("response"), "result": result, "delivery": {"required": True, "mode": "provider_adapter", "status": "ready_for_outbound_adapter"}}

@router.get("/{channel}")
async def verify(channel: str, request: Request):
    if channel not in SUPPORTED_CHANNELS: raise HTTPException(status_code=404, detail="Unsupported channel")
    _verify_token(channel, request); challenge = request.query_params.get("hub.challenge") or request.query_params.get("challenge")
