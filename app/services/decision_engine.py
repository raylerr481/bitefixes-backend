"""BiteFixes Decision Engine V25 — governed reasoning and conversational guardrails."""
...
def _external_consultation(message:str,language:Optional[str],intent:Dict[str,Any],business_context:Optional[Dict[str,Any]])->Dict[str,Any]:
    if not all((evaluate_ai_consultation,consult_ai,ai_provider)) or not ai_provider.available():return {"ai_used":False,"reason":"provider_unavailable"}
    ...
    if not gate.consult:return {"ai_used":False,"reason":gate.reason,"gate_value":gate.estimated_value}
...
def make_decision(company_id:int,customer:Dict,message:str,intent:Dict,knowledge=None,memory=None,language=None,channel="unknown",business_context:Optional[Dict[str,Any]]=None):
    intent=intent or {}
    # Hard conversational guard: a greeting is never allowed to inherit or
    # trigger a service intent returned spuriously by an LLM/classifier.
    ...
    # Promote a semantic follow-up to the active intent even if the LLM emits
    # a weak/mismatched standalone intent. This prevents duplicate tickets.
    ...
    if active_intent and _is_contextual_followup(message,True) and (inherited or float(intent.get("confidence",0) or 0) < 0.70):
        ...
        intent["context_inherited"]=True
        intent["context_source"]="active_conversation"
        intent["confidence"]=max(0.70,float(memory.get("last_confidence",0) or 0))
    ...
    if intent_name and _is_contextual_followup(message, inherited):
        ...
        if intent_name=="mobile_repair":
            response={"pt-BR":"Posso orientar sobre o reparo do celular e a localização. Diga se o problema é tela, bateria, carregamento ou outro.","en":"I can guide you about the phone repair and location. Tell me whether the problem is the screen, battery, charging or something else.","es":"Puedo orientarte sobre la reparación del celular y la ubicación. Dime si el problema es la pantalla, batería, carga u otro."}.get(language,"Puedo orientarte sobre la reparación del celular y la ubicación. Dime qué problema presenta el equipo.")
            service=resolve_service(company_id,intent_name,business_context=business_context)
    ...
    if intent_name in SUPPORT_INTENTS:
        if intent_name=="mobile_repair" and _is_mobile_category(message):
            response={"pt-BR":"Claro. Posso ajudar com celulares. Para começar, me diga o que está acontecendo com o aparelho. Se puder, informe também a marca e o modelo e desde quando ocorre o problema.","en":"Sure. I can help with mobile phones. To start, tell me what is happening with the device. If possible, include the brand, model and when the problem started.","es":"Claro. Puedo ayudarte con celulares. Para comenzar, dime qué le está pasando al equipo. Si puedes, indícame también la marca, el modelo y desde cuándo ocurre el problema."}.get(language,"Claro. Puedo ayudarte con celulares. Dime qué le está pasando al equipo.")
