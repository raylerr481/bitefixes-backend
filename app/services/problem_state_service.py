...
def build_problem_state(history: list[dict[str, Any]], current_message: str) -> dict[str, Any]:
    ...
    current_signals = _score_signals(current)
    prior_signals_scores = _score_signals(prior_user)
    current_signals = list(current_signals)
    ...
    detail_only = bool(user_texts and not current_explicit_symptom and not current_signals and (current_objects or model or location or pending))

    if current_explicit_symptom and current_signals:
        active_category = max(current_signals, key=lambda g: _score_signals(current).get(g, 0))
    elif current_signals:
        active_category = max(current_signals, key=lambda g: _score_signals(current).get(g, 0))
    elif prior_signals and (detail_only or pending_answer):
        active_category = max(prior_signals, key=lambda g: prior_signals_scores.get(g, 0))
    else:
    ...
    # A request with no explicit symptom is a goal, even when it mentions a service object.
    ...
    if different_problem:
        state, active_goal = "NEW_PROBLEM", "SOLVE_PROBLEM"
    elif request_present and not current_explicit_symptom:
    ...
    hypotheses = []
    if active_category:
        score = _score_signals(current).get(active_category) or prior_signals_scores.get(active_category, 1)
    ...
    return {
        ...
        "confirmed_facts":facts, "signal_scores":_score_signals(current) or prior_signals_scores,
