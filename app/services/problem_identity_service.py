...
def extract_device(message:str)->Dict[str,Optional[str]]:
    ...
    for pattern,kind in DEVICE_PATTERNS:
        ...
        if match:
            ...
            platform=next((v for k,v in PLATFORM_WORDS.items() if re.search(rf"\b{re.escape(k)}\b",text)),None)
...
def _semantic_understanding(message:str,active_problem:Optional[str],active_intent:Optional[str],active_device:Optional[str],context:Optional[Dict[str,Any]]=None)->Dict[str,Any]:
    ...
    try:
        result=llm_understand(message=message,language=(context or {}).get("language","es"),context={**(context or {}),"last_problem":active_problem,"active_problem":active_problem,"last_intent":active_intent,"active_device":active_device}) or {}
    ...
    for category,patterns in PROBLEM_PATTERNS.items():
        for phrase in patterns:
            if _norm(phrase) in text:
                scores[category]=scores.get(category,0)+(3 if len(phrase.split())>1 else 2);matched.append(phrase)
    lexical_category=max(scores,key=scores.get) if scores else None
...
def find_customer_problems(customer_id:int,company_id:Optional[int]=None,limit:int=20)->list[dict]:
    ...
    except Exception as error:print("[PROBLEM HISTORY WARNING]",error);return []
