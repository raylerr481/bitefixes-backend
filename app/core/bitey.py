...
def process_message(company_id: int, message: str, phone: str, email: str = "", customer_name: str = "Customer", last_name: str = "", channel: str = "website", conversation_id: Optional[str] = None, language_preference: str = "auto", external_id: str = ""):
    try:
        ...
        if language_preference not in (None, "", "auto") and not explicit_language:
            raise ValueError("Unsupported language_preference")
        language = explicit_language or detect_language(message) or "es"

        # 1) Customer identity is resolved before any problem classification.
        ...
        # 2) Analyze the new message against the customer's problem history.
        ...
        problem = analyze_problem(
            ...
            active_problem=context.get("last_problem"),
            active_device=context.get("last_device"),
        )
        context.update({
            "problem_state": problem["state"],
            "problem_is_new": problem["is_new"],
            "problem_category": problem.get("category"),
            "problem_fingerprint": problem.get("fingerprint"),
            "last_problem": problem.get("category") or context.get("last_problem"),
        })
        if problem.get("device"):
            context["last_device"] = problem["device"]

        # Device/platform evidence has priority over stale intent context.
        ...
        # 3) A new problem must not inherit the previous intent/ticket.
        if problem["is_new"]:
        ...
        if not decision:
            decision = {"action": "conversation", "create_ticket": False, "requires_quote": False, "ticket_type": None, "service": None, "service_id": None, "response": "Claro. Cuéntame un poco más sobre lo que necesitas.", "metadata": {"architecture": "ai_first_v25"}}
        ...
        # 4) Persist the analyzed problem independently of the customer record.
        ...
        if persisted_problem:
            context["last_problem_id"] = persisted_problem.get("id")
            context["last_problem_fingerprint"] = persisted_problem.get("fingerprint")
        ...
        update_conversation_context(resolved_conversation_id, intent=intent.get("intent"), response=response_text, ticket_id=ticket_id, service_id=service_id, confidence=float(intent.get("confidence", 0) or 0), language=language)
        ...
        return {
            ...
            "language": language,
            "problem": problem,
            "problem_id": context.get("last_problem_id"),
            "problem_fingerprint": problem.get("fingerprint"),
            "intent": intent.get("intent"),
            "confidence": float(intent.get("confidence", 0) or 0),
            "memory": {"used": bool(context.get("history")), "messages": len(context.get("history") or []), "problems": len(historical_problems), "scope": "customer+conversation"},
            "decision": decision,
            "ticket": ticket,
            "ticket_id": ticket_id,
            "quote": quote,
