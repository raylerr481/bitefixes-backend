...
# Bounded transport packet. These limits preserve the context volume previously
# tuned for the external AI while preventing accumulated memory/knowledge from
# consuming the user's actual turn. Identity and recent state have priority.
CONTEXT_BUDGET = {
    "business_index": 5500,
    "contextual_state": 4000,
    "contextual_directive": 3000,
    "tool_context": 3500,
    "memory": 3000,
    "knowledge": 5000,
}
...
async def _ask_provider(spec: Any, message: str, language: str, context: Dict[str, Any]) -> Dict[str, Any] | None:
    try:
        ...
        print(f"[AI RECTOR] provider={spec.name} status=requested context_chars={len(prompt)} business_context=yes profile={bool((context.get('business_context') or {}).get('company_ai_profile'))}")
