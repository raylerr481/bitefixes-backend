...
def _normalise_result(item:Dict[str,Any],query:str,company_id:Optional[int]=None)->Optional[ResearchResult]:
    ...
    return ResearchResult(title=title,url=url,snippet=snippet,content=snippet,domain=_domain(url),company_id=company_id,relevance_score=round(relevance,3),authority_score=round(authority,3),verification_score=0.0,freshness_score=1.0,retrieved_at=item.get("retrieved_at") or datetime.now(timezone.utc).isoformat(),metadata={"source":"web_search"})
def _deduplicate(results:Iterable[ResearchResult])->List[ResearchResult]:
    seen=set();output=[]
    for item in results:
...
def _rank_results(results:List[ResearchResult],company_id:Optional[int],verification_score:float=0.0)->List[Dict[str,Any]]:
    enriched=[ResearchResult(**{**item.to_dict(),"verification_score":verification_score}) for item in results]
    ranked=BiteyCME.rank(enriched,company_id=company_id,limit=POLICY.max_results)
...
def _memory_response(memory:Dict[str,Any],message:str,company_id:int)->Dict[str,Any]:
    ...
    for row in memory.get("results",[]):
        results.append(ResearchResult(title=str(row.get("title") or ""),url=str(row.get("canonical_url") or ""),snippet=str(row.get("summary") or row.get("content") or ""),content=str(row.get("content") or row.get("summary") or ""),domain=str(row.get("source_domain") or ""),company_id=company_id,relevance_score=float(row.get("relevance_score") or 0),authority_score=float(row.get("authority_score") or 0),verification_score=float(row.get("verification_score") or 0),freshness_score=1.0 if not row.get("fetched_at") else 1.0,retrieved_at=str(row.get("fetched_at") or ""),metadata={"from_memory":True}))
...
def search_web(message:str,*,intent:Optional[str]=None,limit:int|None=None,company_id:Optional[int]=None)->Dict[str,Any]:
    ...
    evidence=[n for item in raw if(n:=_normalise_result(item,message,company_id))];evidence=_deduplicate(evidence)
    initial=[item.to_dict() for item in evidence]
    verification=_verify(initial,message)
    ranked=_rank_results(evidence,company_id,verification["verification_score"])[:max_results]
