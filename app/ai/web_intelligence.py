...
def _domain_score(url: str) -> float:
    ...
    if host.endswith(".edu") or ".edu." in host:
        return 0.95
    if host in {"microsoft.com", "support.microsoft.com", "learn.microsoft.com", "apple.com", "developer.apple.com", "python.org", "docs.python.org", "supabase.com", "render.com", "github.com", "wordpress.com"}:
        return 0.98
...
def _tokenise(text: str) -> set[str]:
    return {t for t in re.findall(r"[a-z0-9À-ÿ]{3,}", (text or "").lower()) if t not in {"the", "and", "for", "with", "from", "this", "that", "para", "com", "uma", "que"}}
...
def search_web(message: str, *, intent: Optional[str] = None, limit: int | None = None, company_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    for query in queries:
        try:
            result = bitey_search(query, max_results)
            raw.extend(result.get("results") or [])
            if result.get("provider") and result["provider"] not in providers:
