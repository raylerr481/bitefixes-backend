...
The engine compares Bitey Core, knowledge-base and external advisory answers.
It never grants an external provider authority over business actions.
"""
...
_STOPWORDS = {
    ...
    "uma", "um", "e", "em", "com", "no", "na", "ao", "os", "as", "mais",
    "mais", "the", "is", "are", "to", "of", "a", "an", "in", "on", "it",
}
...
def compare_answers(*, message: str, intent: str | None, core_confidence: float,
                    ...
                    min_accept_score: float = 0.48) -> Dict[str, Any]:
    """Rank candidates and choose a governed answer source.

    Core wins ties and remains the default authority. An external answer may be
    selected only when it materially outranks a weak core answer and is marked
    advisory; the returned result is still a recommendation for Bitey Core.
    """
    query_tokens = _tokens(message)
    ranked = [_score(dict(candidate), query_tokens, intent, core_confidence) for candidate in candidates if candidate.get("answer")]
