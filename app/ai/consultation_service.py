...
from app.ai.evaluator import evaluate_suggestions
from app.ai.learning_candidates import record_candidate


def consult_if_valuable(*, company_id: int, message: str, language: str,
    ...
    suggestions = consult(message, language=language, context=context, max_providers=gate.max_providers)
    ...
    if evaluation.get("learning_candidate") and evaluation.get("selected"):
        ...
        record_candidate(company_id=company_id, message=message,
                         provider=selected.get("provider", "unknown"),
                         suggestion=selected, evaluation=evaluation,
                         conversation_id=conversation_id)
    return {"used": bool(suggestions), "gate": gate.__dict__,
            "suggestions": suggestions, "evaluation": evaluation}
